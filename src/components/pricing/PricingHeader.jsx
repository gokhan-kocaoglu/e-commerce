...
import { getPricingHero } from "../../data/pricingData";
import { getPrimaryNav } from "../../data/siteConfig";

export default function PricingHeader({ className = "" }) {
  // Şimdilik tek dil: en
  const hero = getPricingHero("en");
  const nav = getPrimaryNav();

  const homeNav = nav.find((item) => item.id === "home");
  const pricingNav = nav.find((item) => item.id === "pricing");
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-7xl flex-col items-center px-4 text-center">
        ...
        {/* Simple Pricing (desktop & mobile responsive) */}
